...
static char *expand_proxy_command(const char *proxy_command, const char *user,
                                  const char *host, const char *host_arg,
                                  int port) {
  char *tmp, *ret, strport[NI_MAXSERV];
  const char *keyalias =
      options.host_key_alias ? options.host_key_alias : host_arg;

  snprintf(strport, sizeof strport, "%d", port);
  xasprintf(&tmp, "exec %s", proxy_command);
  ret = percent_expand(tmp, "h", host, "k", keyalias, "n", host_arg, "p",
                       strport, "r", options.user, (char *)NULL);
  free(tmp);
  return ret;
}
...
static int ssh_connect_direct(struct ssh *ssh, const char *host,
  ...
  if (want_keepalive &&
      setsockopt(sock, SOL_SOCKET, SO_KEEPALIVE, (void *)&on, sizeof(on)) == -1)
...
int ssh_connect(struct ssh *ssh, const char *host, const char *host_arg,
                struct addrinfo *addrs, struct sockaddr_storage *hostaddr,
                u_short port, int connection_attempts, int *timeout_ms,
                int want_keepalive) {
  int in, out;
  ...
  for (msg = prompt;; msg = fingerprint ? again_fp : again) {
    ...
    if (p[0] == '\0' || strcasecmp(p, "no") == 0)
      ret = 0;
             ...
             (fingerprint != NULL && strcmp(p, fingerprint) == 0))
      ret = 1;
    free(cp);
    if (ret != -1)
      return ret;
...
static char *try_tilde_unexpand(const char *path) {
  char *home, *ret = NULL;
  size_t l;
...
static int hostkeys_find_by_key_cb(struct hostkey_foreach_line *l, void *_ctx) {
  ...
  ctx->names = xrecallocarray(ctx->names, ctx->nnames, ctx->nnames + 1,
                              sizeof(*ctx->names));
...
static int hostkeys_find_by_key_hostfile(const char *file, const char *which,
                                         struct find_by_key_ctx *ctx) {
  int r;

  debug3_f("trying %s hostfile \"%s\"", which, file);
  if ((r = hostkeys_foreach(file, hostkeys_find_by_key_cb, ctx, ctx->host,
                            ctx->ip, HKF_WANT_PARSE_KEY, 0)) != 0) {
...
static char *other_hostkeys_message(const char *host, const char *ip,
                                    const struct sshkey *key,
                                    char **user_hostfiles,
  ...
  hostkeys_find_by_key(host, ip, key, user_hostfiles, num_user_hostfiles,
                       system_hostfiles, num_system_hostfiles, &othernames,
                       &num_othernames);
  if (num_othernames == 0)
...
void load_hostkeys_command(struct hostkeys *hostkeys,
  ...
  if (argv_split(command_template, &ac, &av, 0) != 0) {
    error("%s \"%s\" contains invalid quotes", tag, command_template);
    goto out;
  }
  if (ac == 0) {
    error("%s \"%s\" yielded no arguments", tag, command_template);
    goto out;
  }
  for (i = 1; i < ac; i++) {
    tmp = percent_dollar_expand(
        av[i], DEFAULT_CLIENT_PERCENT_EXPAND_ARGS(cinfo), "H",
        hostfile_hostname, "I", invocation, "t",
        host_key == NULL ? "NONE" : sshkey_ssh_name(host_key), "f",
        key_fp == NULL ? "NONE" : key_fp, "K",
        keytext == NULL ? "NONE" : keytext, (char *)NULL);
    if (tmp == NULL)
...
retry:
  ...
  case HOST_OK:
    ...
    if (want_cert) {
      if (sshkey_cert_check_host(
              host_key,
              options.host_key_alias == NULL ? hostname
  ...
  case HOST_NEW:
    if (options.host_key_alias == NULL && port != 0 &&
      ...
      if (check_host_key(hostname, cinfo, hostaddr, 0, host_key, ROQUIET, 1,
  ...
  case HOST_REVOKED:
    error("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
    error("@          WARNING: HOST KEY GOT 1984'D!                  @");
    error("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
  ...
  case HOST_CHANGED:
    ...
    if (options.check_host_ip && host_ip_differ) {
      ...
      error("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
      error("@     WARNING: FUNNY BUSINESS ON THE DNS DETECTED!        @");
      error("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
...
int verify_host_key(char *host, struct sockaddr *hostaddr,
  ...
  if (sshkey_equal(previous_host_key, host_key)) {
    debug2_f("server host key %s %s matches cached key", sshkey_type(host_key),
             fp);
    r = 0;
    goto out;
  }
  ...
  r = check_host_key(host, cinfo, hostaddr, options.port, host_key, RDRW, 0,
...
static int show_other_keys(struct hostkeys *hostkeys, struct sshkey *key) {
  ...
  for (i = 0; type[i] != -1; i++) {
    if (type[i] == key->type)
      continue;
    if (!lookup_key_in_hostkeys_by_type(hostkeys, type[i], -1, &found))
      continue;
...
static void warn_changed_key(struct sshkey *host_key) {
  ...
  error("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
  error("@            WARNING: REMOTE HOST HAS CANCER!             @");
  error("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
...
void maybe_add_key_to_agent(const char *authfile, struct sshkey *private,
  ...
  if ((r = ssh_add_identity_constrained(
           auth_sock, private, comment == NULL ? authfile : comment,
           options.add_keys_to_agent_lifespan, (options.add_keys_to_agent == 3),
           0, skprovider, NULL, 0)) == 0)
